...
proof
  ...
  have S:"(least_comp r n S) \<in> S" unfolding least_comp_def using assms leastn_comp_single leastn_comp_sub least_comp_in least_comp_def by metis  have "length (least_comp r n S) \<le> length x" using S using assms(3) x by force
...
proof
  ...
  proof(cases "x \<in> least_length S")
    ...
    moreover have "(least_length S) \<subseteq> \<langle>A\<rangle>" using assms(2) least_length_sub by blast
  ...
  next
    case False
    have LS: "(least_length S) \<subseteq> \<langle>A\<rangle>" using assms(2) least_length_sub by blast
    have 1:"(length ` S) \<noteq> {}" using assms(3) by auto
    then have "(least (nat_leq_less) (length ` S)) \<in> (length ` S)" using least_def nat_unique_least by (smt (verit, del_insts) assms image_is_empty theI)
