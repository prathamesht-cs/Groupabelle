...
proof-
  (**)
  have "\<forall>x y. (x, y) \<in> (lex_set \<langle>A\<rangle> r) \<longrightarrow> (y, x) \<in> (lex_set \<langle>A\<rangle> r) \<longrightarrow> x = y" sorry
  then show ?thesis using antisym_def by blast
qed
