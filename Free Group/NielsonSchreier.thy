...
lemma min_L2_inv:
  assumes "x ∈ carrier (freegroup A) ∧ xy ∈ carrier (freegroup A)"
...
lemma lex_word_one: "(x,y) ∈ lex_word ⟹ x ≠ y ⟹ ¬ (y,x) ∈ lex_word"
...
lemma lex_L2_word_total_1:
  assumes "x ∈ carrier (freegroup A)"
      and "y ∈ carrier (freegroup A)"
      and "length (red_rep A x) = length (red_rep A y)"
    shows "¬ (x,y) ∈ lex_L2_word A ∧ ¬ (y, x) ∈ lex_L2_word A ⟹ red_rep A x = red_rep A y ∨ red_rep A x = wordinverse (red_rep A y)"
...
lemma lex_L2_word_total_2:
  assumes "x ∈ carrier (freegroup A)"
      and "y ∈ carrier (freegroup A)"
      and "length (red_rep A x) = length (red_rep A y)"
    shows "red_rep A x ≠ red_rep A y ∧ red_rep A x ≠ wordinverse (red_rep A y) ⟹ (x,y) ∈ lex_L2_word A ∨ (y, x) ∈ lex_L2_word A"
  using assms lex_L2_word_total_1 by blast

lemma lex_total:
  assumes "x ∈ carrier (freegroup A)"
      and "y ∈ carrier (freegroup A)" 
      and "red_rep A x ≠ wordinverse (red_rep A y)"
      and "red_rep A x ≠ (red_rep A y)"
  shows "(x,y) ∈ lex_L2_word A ∨ (y, x) ∈ lex_L2_word A"
proof(cases "length (red_rep A x) > length (red_rep A y)")
  case True 
  then have "(y,x) ∈ lex_L2_word A" using assms(1) assms(2) length_lex by blast
...
next
  ...
  proof(cases "length (red_rep A x) < length (red_rep A y)")
    case True
    then have "(x,y) ∈ lex_L2_word A" using assms(1) assms(2) length_lex by blast
    then show ?thesis by blast
  next
    case False
...
lemma reduced_inv_eq_imp_nil: "xs = wordinverse xs ⟹ reduced xs ⟹ xs = []"
proof-
  ...
  proof(cases "odd (length xs)")
    case True
      ...
      then have "(drop (length xs div 2) xs) ≠ []" by fast
  ...
  next
    ...
    then have  "¬ reduced xs ∨ xs = []" by (metis inverse_wordinverse reduced.simps(1) reduced_reln_eq)
...
lemma square_length:
  assumes "x ∈ carrier (freegroup A)"
  shows "length (red_rep A (x ⊗⇘ F⇘A⇙⇙ x)) ≥ length (red_rep A x)"
proof-
  let ?x = "(red_rep A x)"
  let ?xx = "(cancel2 ?x ?x)"
  have xx: "(x ⊗⇘F⇘A⇙⇙ x) ∈ carrier (freegroup A)" by (simp add: assms(1) freegroup_is_group group.subgroupE(4) group.subgroup_self)
  have 1:"reduced ?x" using assms(1) red_rep_def red_rep_the unfolding freegroup_def by fastforce
  then have "((red_rep A x) @ (red_rep A x)) ~ ((a2 ?xx) @ (b2 ?xx))" by (metis cancel2_reln cancel2_the)
  then have "(red_rep A (x ⊗⇘F⇘A⇙⇙ x)) ~ ((a2 ?xx) @ (b2 ?xx))" using assms mult_reln using reln.trans by blast
  moreover have "reduced ((a2 ?xx) @ (b2 ?xx))" by (simp add: "1" cancel2_the)
  moreover have "reduced (red_rep A (x ⊗⇘F⇘A⇙⇙ x))" using xx red_rep_def red_rep_the unfolding freegroup_def by fastforce
  ultimately have 3: "(red_rep A (x ⊗⇘F⇘A⇙⇙ x)) = ((a2 ?xx) @ (b2 ?xx))" by (simp add: reduced_reln_eq)
  ...
  proof(cases "length (b2 ?xx) > length (p2 ?xx)")
    case True
    then have "length ((a2 ?xx) @ (p2 ?xx)) ≤ length ((a2 ?xx) @ (b2 ?xx))" by simp
    then show ?thesis using 3 A by auto
  next
    case False
    then have F:"length (b2 ?xx) ≤ length (p2 ?xx)" by auto
    then show ?thesis
    proof(cases "length (b2 ?xx) = length (p2 ?xx)")
      case True
      then have "?x = wordinverse (p2 ?xx) @ (p2 ?xx)" by (metis A B D append_eq_append_conv)
      then have "¬ reduced ?x ∨ ?x = []"  by (metis inverse_wordinverse reduced.simps(1) reduced_reln_eq)
...
lemma neq_N1:
  assumes "x ∈ carrier (freegroup A)" 
    and " y ∈ carrier (freegroup A)"
    and "length (red_rep A (x ⊗⇘ F⇘A⇙⇙ y)) < length (red_rep A x) ∨ length (red_rep A (x ⊗⇘ F⇘A⇙⇙ y)) <  length (red_rep A y)"
  shows "red_rep A x ≠ red_rep A y"
proof(rule ccontr)
  assume "¬ red_rep A x ≠ red_rep A y"
  ...
  then have "length (red_rep A (x ⊗⇘ F⇘A⇙⇙ y)) ≥ length (red_rep A x) ∧ length (red_rep A (x ⊗⇘ F⇘A⇙⇙ y)) ≥  length (red_rep A y)" using assms(1) square_length by auto
...
lemma notin_union_inv:
  assumes "H ≤ (freegroup A)" "x ∉ S" "m_inv (SG (freegroup A) H) x ∉ S" "S ⊆ H"
  shows "x ∉ union_inv S A"
proof(rule ccontr)
  assume "¬ x ∉ union_inv S A"
  then have "x ∈ union_inv S A"  by blast
  then have "x ∈ S ∨ x ∈ m_inv F⇘A⇙ ` S" unfolding union_inv_def by auto
  then have c:"x ∈ S ∨ x ∈ m_inv (SG (freegroup A) H) ` S" using inv_SG freegroup_is_group assms(1,4) by (metis (no_types, lifting) image_cong subset_eq)
  show False
  proof(cases "x ∈ S")
    case True
  ...
  next
    case False
    then have "x ∈ m_inv (SG (freegroup A) H) ` S" using c by blast
    then obtain y where y:"x = m_inv (SG (freegroup A) H) y ∧ y ∈ S" by blast
    moreover then have "y ∈ H" using assms(4) by auto
    moreover then have "y ∈ carrier (SG (freegroup A) H)" unfolding SG_def by simp
...
proof-
  ...
  proof(rule ccontr)
    assume N1: "¬ N1 x y"
    then have nxiy:"x ≠ wordinverse y" using N1_def by auto
    ...
    have "¬ (length (red_rep A (x1 ⊗⇘ F⇘A⇙⇙ y1)) ≥ length (red_rep A x1) ∧ length (red_rep A (x1 ⊗⇘ F⇘A⇙⇙ y1)) ≥ length (red_rep A y1))" using N1 x1 y1 y1A x1A length_N1 by blast
    then have t:"length (red_rep A (x1 ⊗⇘ F⇘A⇙⇙ y1)) < length (red_rep A x1) ∨ length (red_rep A (x1 ⊗⇘ F⇘A⇙⇙ y1)) <  length (red_rep A y1)" by auto
    ...
    have nxy:"x ≠ y" using neq_N1 t x1A y1A x1 y1 by auto
    have XH: "(X (SG F⇘A⇙ H) A) ⊆ H" unfolding X_def SG_def by simp
    have "x1 ∉ (union_inv (X (SG (freegroup A) H) A) A) ∨ y1 ∉ (union_inv (X (SG (freegroup A) H) A) A)"
  proof(cases "((x1 ⊗⇘F⇘A⇙⇙ y1), x1) ∈ lex_L2_word A")
    case True note xy_x = this
    then have subcases: "(x1,y1) ∈ lex_L2_word A ∨ (y1, x1) ∈ lex_L2_word A" using lex_total nxy nxiy x1 y1 x1A y1A  by auto
    then show ?thesis 
    proof (cases "(x1,y1) ∈ lex_L2_word A")
      case True
      then have xy_y:"((x1 ⊗⇘F⇘A⇙⇙ y1), y1) ∈ lex_L2_word A" using xy_x trans_lex_L2_word unfolding trans_def by blast
      ...
      ultimately have "y1 ∉ (union_inv (X (SG (F⇘A⇙) H) A) A)" using notin_union_inv XH assms by blast
    ...
    next
      ...
      ultimately have "x1 ∉ (union_inv (X (SG (F⇘A⇙) H) A) A)" using notin_union_inv XH assms by blast
      then show ?thesis by blast
    qed
  next
    case False
    ...
    next
      ...
      then have xy_x: "((x1 ⊗⇘F⇘A⇙⇙ y1), x1) ∈ lex_L2_word A" using xyy trans_lex_L2_word unfolding trans_def by blast
...
lemma L_inv_eq: "L(xs) = L(wordinverse xs) ⟹ xs ≠ [] ⟹ ¬ (reduced xs)"
proof-
  assume xs: "L(xs) = L(wordinverse xs)" and rxs:"xs ≠ []"
  then show "¬ (reduced xs)"
  proof-
    have "L2 xs = ↓ (L2 xs)" unfolding left_tuple_def rev_tuple.simps using xs by simp
...
lemma lex_word_init:
  "(x, y) ∈ lex_word ⟹ (length a = length b) ⟹ (x@a, y@b) ∈ lex_word"
  unfolding lex_word_def by (simp add: lenlex_append1)

lemma left_includes: "a = x @ y ⟹ length x ≤ length y ⟹ ∃z. L a = x @ z"
  unfolding left_subword_def by (simp add: take_append take_length)

lemma take_bigger_half:"length a ≥ length  b ⟹ take (((length (a@b)+1) div 2)) (a@b) = take (((length (a@b)+1) div 2)) a"
  by simp
  
lemma lex_word_rightappend:
  assumes "(x,y) ∈ lex_word"
  shows "(x@c, y@c) ∈ lex_word"
...
lemma L_inverse_eq:
  assumes "x = (p @ (wordinverse a))"
          ...
          and "length p ≤ length (wordinverse a)"
          and "length q ≤ length (wordinverse a)"
...
proof-
  ...
  moreover have "length (wordinverse  p) ≤ length a" by (metis assms(4) length_wordinverse)
  ...
  moreover have "length (wordinverse  q) ≤ length a" by (metis assms(5) length_wordinverse)
...
lemma neq_left_neq: "p ≠ q ⟹ length p = length q ⟹ length p ≤ length r ⟹ L (p @ r) ≠ L (q @ r)"
  unfolding left_subword_def by simp

lemma lex_L2_inv2:
  assumes "(y,x) ∈ lex_L2_word A"
  shows "(inv⇘freegroup A⇙ y, x) ∈ lex_L2_word A"
proof-
  have 1:"y ∈ (⟨A⟩ // (reln_tuple ⟨A⟩))" using assms(1) unfolding lex_L2_word_def by blast
  then obtain invx where "invx = (inv⇘freegroup A⇙ y)" using freegroup_is_group by simp
  then have x:"(inv⇘freegroup A⇙ y) ∈ (⟨A⟩ // (reln_tuple ⟨A⟩))" using m_inv_def[of "freegroup A" "y"] freegroup_def
    by (metis (no_types, lifting) freegroup_is_group group.inv_closed partial_object.select_convs(1) 1)
  have y: "x ∈ (⟨A⟩ // (reln_tuple ⟨A⟩))" using assms(1) unfolding lex_L2_word_def by blast
  have 2:"(length (red_rep A y) < length (red_rep A x)) ∨ ((length (red_rep A y) = length (red_rep A x) ∧ (y,x) ∈ lex_L2_word' A))" 
    using nat_less_def assms unfolding lex_L2_word_def lex_prod_def by fastforce
  then show ?thesis 
  proof(cases "(length (red_rep A y) < length (red_rep A x))")
    ...
    then have "length (red_rep A (inv⇘freegroup A⇙ y)) < length (red_rep A x)" using 1 red_rep_inv by metis
    then show ?thesis using x y by (simp add: lex_L2_word_def nat_less_def)
  next
    case False
    then have 3:"((length (red_rep A y) = length (red_rep A x) ∧ (y,x) ∈ lex_L2_word' A))" using 2 by blast
    then have 4:"length (red_rep A (inv⇘freegroup A⇙ y)) = length (red_rep A x)" using 1 red_rep_inv by (metis length_wordinverse)
    then have "((λx. (min lex_word (L2 (red_rep A x)), max lex_word (L2 (red_rep A x)))) y 
                , (λx. (min lex_word (L2 (red_rep A x)), max lex_word (L2 (red_rep A x)))) x) ∈ (lex_word <*lex*> lex_word)" using 3 unfolding lex_L2_word'_def by fastforce
    then have 5:"((min lex_word (L2 (red_rep A y))), (min lex_word (L2 (red_rep A x)))) ∈ lex_word ∨ 
               (min lex_word (L2 (red_rep A y))) = (min lex_word (L2 (red_rep A x))) ∧ 
    ...
    then have L2_winv:"min lex_word (L2 (red_rep A y)) = min lex_word (L2 (wordinverse (red_rep A y))) ∧
    ...
    then have "((min lex_word (L2 (wordinverse(red_rep A y)))), (min lex_word (L2 (red_rep A x)))) ∈ lex_word ∨ 
               (min lex_word (L2 (wordinverse(red_rep A y)))) = (min lex_word (L2 (red_rep A x))) ∧ 
               (max lex_word (L2 (wordinverse(red_rep A y))), max lex_word (L2 (red_rep A x))) ∈ lex_word" using 5 by auto   
    then have "((min lex_word (L2 (red_rep A(inv⇘freegroup A⇙ y)))), (min lex_word (L2 (red_rep A x)))) ∈ lex_word ∨
               (min lex_word (L2 (red_rep A(inv⇘freegroup A⇙ y)))) = (min lex_word (L2 (red_rep A x))) ∧ 
               (max lex_word (L2 (red_rep A(inv⇘freegroup A⇙ y))), max lex_word (L2 (red_rep A x))) ∈ lex_word" using red_rep_inv 1 by force    
    then have "((inv⇘freegroup A⇙ y),x) ∈ lex_L2_word' A" unfolding lex_L2_word'_def using x y by auto
    then show ?thesis using x y 2 4 lex_L2_word_length by blast
  qed
qed
...
lemma three_point_six:
  assumes "x ∈ carrier (freegroup A) ∧ xy ∈ carrier (freegroup A)"      
          "red_rep A x =  p@c ∧ red_rep A xy = q@c"
          "length p = length q" "length p ≤ length c" "length q ≤ length c"    
          "(p,q) ∈ lex_word" "p ≠ q" "N0 (red_rep A x)" "N0 (red_rep A xy)"
  shows   "(x, xy) ∈ lex_L2_word A"
proof-
  let ?X = "(red_rep A x)"
  let ?XY = "(red_rep A xy)"
  ...
  have "L(?X) ≠ L (?XY)" using assms(2,3,4,7) neq_left_neq by fastforce
  ...
  ultimately have "((p@r), (q@s)) ∈ lex_word" using assms(6) lex_word_def lenlex_append1 by blast
  then have L_lex:"(L(?X), L(?XY)) ∈ lex_word" using r s by simp
  ...
  show ?thesis
  proof(cases "(L(wordinverse ?X), L(?X)) ∈ lex_word")
    case True
    ...
    then have 1:"(L(wordinverse ?XY), L(?XY)) ∈ lex_word" unfolding left_tuple_def min.simps using L_eq L_lex True lex_word_one lex_word_total by fastforce
    ...
    have "x ∈ ⟨A⟩ // reln_tuple ⟨A⟩ ∧ xy ∈ ⟨A⟩ // reln_tuple ⟨A⟩" using assms(1) freegroup_def[of "A"] by auto
    moreover have "(min lex_word (L2 (?X))) = (min lex_word (L2 (?XY)))" using min1 min2 L_eq by auto
    moreover have "((max lex_word (L2 (wordinverse (?X))), max lex_word (L2 (wordinverse (?XY))))) ∈ lex_word" using max1 max2 L_lex by (metis FreeGroupMain.wordinverse_symm True left_tuple_def max.simps)
    ultimately have "(x, xy) ∈ lex_L2_word' A" unfolding lex_L2_word'_def lex_prod_def left_tuple_def using wordinverse_symm left_tuple_def by (smt (z3) case_prod_conv  max.simps max1 mem_Collect_eq min.simps min1)
    then show ?thesis by (simp add: ‹x ∈ ⟨A⟩ // reln_tuple ⟨A⟩ ∧ xy ∈ ⟨A⟩ // reln_tuple ⟨A⟩› length lex_L2_word_length)     
   next
    case False
    then have "(L(?X),L(wordinverse ?X)) ∈ lex_word" using L_inv_eq N0_def assms(8) lex_word_total rx by auto
    ...
    then show ?thesis
    proof(cases "(L(wordinverse ?XY), L(?XY)) ∈ lex_word")
      case True
      then have "min lex_word (L2 (wordinverse ?XY)) = L(wordinverse ?XY)" unfolding left_tuple_def min.simps by (simp add: wordinverse_of_wordinverse)
      then have min2:"min lex_word (L2 (?XY)) = L(wordinverse ?XY)" using min_L2_inv using assms(1) by fastforce
      then have max2:"max lex_word (L2 (wordinverse ?XY)) = L(?XY)" by (metis FreeGroupMain.wordinverse_symm True left_tuple_def max.simps)
      have "x ∈ ⟨A⟩ // reln_tuple ⟨A⟩ ∧ xy ∈ ⟨A⟩ // reln_tuple ⟨A⟩" using assms(1) freegroup_def[of "A"] by auto
      moreover have "((min lex_word (L2 (red_rep A x))),(min lex_word (L2 (red_rep A xy)))) ∈ lex_word" using min1 min2 max2 L_eq using ‹(L (red_rep A x), L (wordinverse (red_rep A x))) ∈ lex_word› by auto
      ultimately have "(x, xy) ∈ lex_L2_word' A" unfolding lex_L2_word'_def lex_prod_def left_tuple_def using wordinverse_symm left_tuple_def by (smt (z3) case_prod_conv  max.simps max1 mem_Collect_eq min.simps min1)
      then show ?thesis unfolding lex_L2_word'_def using assms(1,6,7) freegroup_def length ‹(x, xy) ∈ lex_L2_word' A› lex_L2_word_length by blast
    next
      case False
      then have subcases:"(L(wordinverse ?XY) = L(?XY)) ∨ (L(?XY),(L(wordinverse ?XY))) ∈ lex_word" using lex_word_total by blast
      then show ?thesis
      ...
      next
        case False
        then have false:"(L(?XY),(L(wordinverse ?XY))) ∈ lex_word" using subcases by simp
        then have 2:"min lex_word (L2 ?XY) = L(?XY)" unfolding left_tuple_def min.simps by (simp add: wordinverse_of_wordinverse)
        have 1:"(L ?XY, L (wordinverse ?X)) ∈ lex_word" using false L_eq by simp 
        have "x ∈ ⟨A⟩ // reln_tuple ⟨A⟩ ∧ xy ∈ ⟨A⟩ // reln_tuple ⟨A⟩" using assms(1) freegroup_def[of "A"] by auto
        moreover have "((min lex_word (L2 (red_rep A x))),(min lex_word (L2 (red_rep A xy)))) ∈ lex_word" using 1 2 false L_eq min1 using L_lex by auto
        ultimately have "(x, xy) ∈ lex_L2_word' A" unfolding lex_L2_word'_def lex_prod_def left_tuple_def using wordinverse_symm left_tuple_def by (smt (z3) case_prod_conv  max.simps max1 mem_Collect_eq min.simps min1)    
       then show ?thesis unfolding lex_L2_word'_def using assms(1,6,7) freegroup_def length ‹(x, xy) ∈ lex_L2_word' A› lex_L2_word_length by blast
      qed
    qed
...
lemma three_point_seven:assumes "x ∈ (carrier (freegroup A))"
          and "xy ∈ (carrier (freegroup A))"
          and "red_rep A x = (a @ (wordinverse p))"
          and "red_rep A xy = (a @ (wordinverse q))"
          and "length (wordinverse p) = length (wordinverse q)"
          and "length (wordinverse p) ≤ length a"
          and "length (wordinverse q) ≤ length a"
          and "(q, p) ∈ lex_word"
          and "p ≠ q"
          and "(red_rep A x) ≠ []"
          and "(red_rep A xy) ≠ []"
        shows "(xy, x) ∈ lex_L2_word A"
proof-
  ...
  have "L (?XY) ≠ L (?X)" using xy x neq_left_neq assms(5,9) assms(7) by (metis length_wordinverse) 
  have pq:"length p = length q" using assms(5) by (metis length_wordinverse)
  have p:"length p ≤ length (wordinverse a)" by (metis assms(6) length_wordinverse)
  then obtain r where r:"p @ r = L (?X)" using x left_includes by metis
  have "length q ≤ length (wordinverse a)" by (metis assms(7) length_wordinverse)
  ...
  ultimately have "((q@s), (p@r)) ∈ lex_word" by (simp add: lex_word_init assms(8))
  then have L:"((L ?XY),(L ?X)) ∈ lex_word" by (simp add: r s)
  have R:"L (wordinverse ?X) = L (wordinverse ?XY)" using L_inverse_eq x xy pq assms(6) assms(7) p by fastforce
  have Xneq: "L (?X) ≠ L (wordinverse ?X)"
  proof(rule ccontr)
    assume "¬ L (red_rep A (inv⇘F⇘A⇙⇙ x)) ≠ L (wordinverse (red_rep A (inv⇘F⇘A⇙⇙ x)))"
    then have "L (red_rep A (inv⇘F⇘A⇙⇙ x)) = L (wordinverse (red_rep A (inv⇘F⇘A⇙⇙ x)))" by blast
  ...
  qed
  have XYneq: "L (?XY) ≠ L (wordinverse ?XY)"
  proof(rule ccontr)
    assume "¬ L (red_rep A (inv⇘F⇘A⇙⇙ xy)) ≠ L (wordinverse (red_rep A (inv⇘F⇘A⇙⇙ xy)))"
    then have "L (red_rep A (inv⇘F⇘A⇙⇙ xy)) = L (wordinverse (red_rep A (inv⇘F⇘A⇙⇙ xy)))" by blast
  ...
  proof(cases "(L (?XY), L (wordinverse ?XY)) ∈ lex_word")
    ...
    then show ?thesis
    proof(cases "(L (?X), L (wordinverse ?X)) ∈ lex_word")
      case True
      then have "(min lex_word (L2 ?X)) = L ((?X))" unfolding left_tuple_def min.simps by simp
      then have "((min lex_word (L2 ?XY)), min lex_word (L2 ?X)) ∈ lex_word" using L 1 by fastforce
    ...
    next
      ...
      moreover have "(L ?XY, L (wordinverse ?X)) ∈ lex_word" using R first by simp 
      ultimately have "((min lex_word (L2 ?XY)), min lex_word (L2 ?X)) ∈ lex_word" using R 1 by fastforce
      then show ?thesis unfolding lex_L2_word'_def lex_prod_def using xin xyin by fast
    qed    
  next
    case False
    then have 1: "(L (wordinverse ?XY),L ?XY) ∈ lex_word" using XYneq lex_word_total by auto
    have A:"(min lex_word (L2 ?XY)) = L (wordinverse ?XY)" unfolding left_tuple_def min.simps using False by simp
    have 2:"(L (wordinverse ?X),L (?X)) ∈ lex_word" using 1 L R trans_lex_word transD by fastforce
    ...
    ultimately have "((max lex_word (L2 ?XY)), max lex_word (L2 ?X)) ∈ lex_word" using L by simp
    then show ?thesis unfolding lex_L2_word'_def lex_prod_def using xin xyin min by auto
  qed
  ultimately have "((m_inv (freegroup A) xy, m_inv (freegroup A) x) ∈ lex_L2_word A)" by simp
  then have "(m_inv (freegroup A) (m_inv (freegroup A) xy), m_inv (freegroup A) x) ∈ lex_L2_word A" by (simp add: lex_L2_inv2)
  then have "(m_inv (freegroup A) (m_inv (freegroup A) xy), m_inv (freegroup A) (m_inv (freegroup A) x)) ∈ lex_L2_word A" by (simp add: lex_L2_inv)
  then show ?thesis using assms(1,2) by (simp add: freegroup_is_group)
qed

lemma N0:
  assumes "H ≤ freegroup A" 
  shows "∀x ∈ (red_rep A) ` (union_inv (X (SG (freegroup A) H) A) A). N0 x"
proof
  fix x assume x:"x ∈ red_rep A ` union_inv (X (SG F⇘A⇙ H) A) A"
  show "N0 x"
  proof(rule ccontr)
    assume "¬ N0 x"
    then have "x = []" unfolding N0_def by simp
    moreover obtain x1 where x1:"red_rep A x1 = x ∧ x1 ∈ (union_inv (X (SG (freegroup A) H) A) A)" using x by blast
    moreover then have xin:"x1 ∈ carrier(freegroup A)" using assms assms union_inv_clos by blast
    ultimately have 1:"x1 = 𝟭⇘freegroup A⇙" unfolding red_rep_def freegroup_def using red_rep_the x1 by force
    then have "x1 ∈ (X (SG (freegroup A) H) A) " 
    proof(cases "x1 ∈ (X (SG (freegroup A) H) A)")
      case True
      then show ?thesis  by simp
    next
      case False
      then have "x1 ∈ m_inv (freegroup A) ` (X (SG (freegroup A) H) A)" using x1 unfolding union_inv_def  by simp
      then obtain x2 where x2:"m_inv (freegroup A) x2 = x1 ∧ x2 ∈ (X (SG (freegroup A) H) A)" by blast
      moreover then have "x2 ∈ carrier(freegroup A)"  using assms union_inv_clos union_inv_def by fastforce
      ultimately have "m_inv (freegroup A) x1 = x2" using freegroup_is_group group.inv_inv  by fast
      then have "x2 = 𝟭⇘freegroup A⇙" using 1 freegroup_is_group by (metis group.inv_eq_1_iff xin)
      then show ?thesis using x2 1 by simp
    qed
    moreover have "𝟭⇘freegroup A⇙ ∈ G (SG (freegroup A) H) A (𝟭⇘freegroup A⇙)" unfolding G_def by (metis (no_types, lifting) gen_span.simps one_SG)
...
proof-
  ...
  define w2 where 3:"w2 = ((wordinverse  (p3 (cancel3 x y z)) @ b3 (cancel3 x y z)), (q3 (cancel3 x y z)), (c3 (cancel3 x y z)))"
  then have "y = (a2 w2) @ (p2 w2) ∧ z = (wordinverse (p2 w2)) @ (b2 w2) ∧ reduced ((a2 w2) @ (b2 w2))" using 1 by auto
  then have "w2 = (cancel2 y z)" using 2 assms(2,3) cancel2E' by blast
...
lemma neq_imp_invneq: "p ≠ q ⟹ wordinverse p ≠ wordinverse q"
...
lemma red_repI: assumes "x1 ∈ ⟨A⟩ // reln_tuple ⟨A⟩"
              "x1 = reln_tuple ⟨A⟩ `` {x}"
              "reduced x" 
            shows "red_rep A x1 = x" 
proof-
  have "x ∈ x1" 
    by (metis Image_singleton_iff assms(1) assms(2) equiv_class_eq_iff red_rep_the reln_equiv)
  then have "x ∈ x1 ∧ reduced x ∧ x1 = reln_tuple ⟨A⟩ `` {x}" using assms by auto
  then show ?thesis  using assms(1) red_rep_the[of "x1" "A"] unfolding red_rep_def using redelem_unique by fastforce
qed

lemma N2:
  assumes "H ≤ freegroup A" 
    and "∀x ∈ (red_rep A) ` (union_inv (X (SG (freegroup A) H) A) A). N0 x"
    and "∀x ∈ (red_rep A) ` (union_inv (X (SG (freegroup A) H) A) A). ∀y ∈ (red_rep A) ` (union_inv (X (SG (freegroup A) H) A) A). N1 x y"
  shows "∀x ∈ (red_rep A) ` (union_inv (X (SG (freegroup A) H) A) A). ∀y ∈ (red_rep A) ` (union_inv (X (SG (freegroup A) H) A) A). ∀z ∈ (red_rep A) ` (union_inv (X (SG (freegroup A) H) A) A). N2 x y z"
  apply(rule ballI)+
proof-
  fix x y z assume x: "x ∈ red_rep A ` union_inv (X (SG F⇘A⇙ H) A) A" and  y: "y ∈ red_rep A ` union_inv (X (SG F⇘A⇙ H) A) A" and z:"z ∈ red_rep A ` union_inv (X (SG F⇘A⇙ H) A) A"
  show "N2 x y z"
  proof(rule ccontr)
    assume N2: "¬ N2 x y z"
    then have invxyz:"x ≠ wordinverse y ∧ y ≠ wordinverse z" using N2_def by auto
    obtain x1 where x1:"red_rep A x1 = x ∧ x1 ∈ (union_inv (X (SG (freegroup A) H) A) A)" using x by blast
    ...
    then have xyz:"x = a3 (⊘⇘3⇙ x y z) @ p3 (⊘⇘3⇙ x y z) ∧
           y = wordinverse (p3 (⊘⇘3⇙ x y z)) @ q3 (⊘⇘3⇙ x y z) ∧
           z = wordinverse (q3 (⊘⇘3⇙ x y z)) @ c3 (⊘⇘3⇙ x y z) ∧
           reduced (a3 (⊘⇘3⇙ x y z) @ q3 (⊘⇘3⇙ x y z)) ∧
           reduced (wordinverse (p3 (⊘⇘3⇙ x y z)) @ c3 (⊘⇘3⇙ x y z))" using assms(3) rx ry rz x y z cancel3_the invxyz by (metis append.left_neutral)
    then have neq:"(p3 (⊘⇘3⇙ x y z)) ≠ q3 (⊘⇘3⇙ x y z)" using y assms(2) ry by (metis FreeGroupMain.inverse_wordinverse N0_def reduced.simps(1) reduced_reln_eq)
    have pa:"length (p3 (⊘⇘3⇙ x y z)) ≤ length (a3 (⊘⇘3⇙ x y z))" using assms(3) xyz rx ry rz x y z invxyz cancel2_the cancel_a2_a3 unfolding N1_def by (metis (no_types, lifting)  same_append_eq)
    have "length (p2 (y ⊘⇘2⇙ z)) ≤ length (b2 (y ⊘⇘2⇙ z))" using assms(3) ry rz y z xyz invxyz cancel2_the N1_def by blast
    then have qc: "length (q3 (⊘⇘3⇙ x y z)) ≤ length (c3 (⊘⇘3⇙ x y z))" using assms(3) xyz rx ry rz x y z invxyz cancel_b2_c3 cancel_p2_q3 by (metis (no_types, lifting))
    have "length (p2 (x ⊘⇘2⇙ y)) ≤ length (b2 (x ⊘⇘2⇙ y))"  using assms(3) rx ry x y xyz invxyz cancel2_the N1_def by blast
    then have pq: "length (p3 (⊘⇘3⇙ x y z)) ≤ length (q3 (⊘⇘3⇙ x y z))" using assms(3) xyz rx ry rz x y z invxyz cancel_b2_bq3 cancel_p2_p3 b by (metis append.left_neutral)
    have "length (p2 (y ⊘⇘2⇙ z)) ≤ length (b2 (y ⊘⇘2⇙ z))"  using assms(3) ry rz y z xyz invxyz cancel2_the N1_def by blast
    then have qp: "length (q3 (⊘⇘3⇙ x y z)) ≤ length (p3 (⊘⇘3⇙ x y z))" using assms(3) xyz rx ry rz x y z invxyz cancel_p2_q3 b by (metis N1_def append_Nil2 cancel_a2_pb3 length_wordinverse)
    then have leq:"length (q3 (⊘⇘3⇙ x y z)) = length (p3 (⊘⇘3⇙ x y z))" using pq by simp
    then have pc: "length (p3 (⊘⇘3⇙ x y z)) ≤ length (c3 (⊘⇘3⇙ x y z))" using qc by simp
    have qa: "length (q3 (⊘⇘3⇙ x y z)) ≤ length (a3 (⊘⇘3⇙ x y z))" using pa leq by simp
    ...
    show False
    proof(cases "(wordinverse (p3 (⊘⇘3⇙ x y z)), wordinverse (q3 (⊘⇘3⇙ x y z))) ∈ lex_word")
      case True
      let ?p = "wordinverse (p3 (⊘⇘3⇙ x y z))"
      let ?q = "wordinverse (q3 (⊘⇘3⇙ x y z))"
      let ?c = "c3 (⊘⇘3⇙ x y z)"
      have "red_rep A z1 = ?q @ ?c"  using xyz z1 by force
      moreover have "red_rep A (y1 ⊗⇘freegroup A⇙ z1) = ?p @ ?c" by (simp add: y1z1)
      moreover have "length ?p = length ?q" using leq by (metis length_wordinverse)
      moreover have "?p ≠ ?q" using ineq .
      moreover have "length ?p ≤ length ?c" by (metis length_wordinverse pc)
      moreover have "length ?q ≤ length ?c" by (metis length_wordinverse qc)
      moreover have "red_rep A (y1 ⊗⇘freegroup A⇙ z1) ≠ []" using y1z1 FreeGroupMain.wordinverse_symm invxyz xyz by fastforce
      moreover have "red_rep A z1 ≠ []" using N0_def assms(2) z1 by auto
      ultimately have cont:"((y1 ⊗⇘freegroup A⇙ z1), z1) ∈ lex_L2_word A" using True z1A y1z1in xyz z three_point_six[of "z1" "A" "(y1 ⊗⇘freegroup A⇙ z1)" "?p" "?c" "?q"] by (metis N0_def three_point_six)
      have subcases:"(y1, z1) ∈ lex_L2_word A ∨ (z1, y1) ∈ lex_L2_word A" by (simp add: invxyz lex_total y1 y1A yneqz z1 z1A)      
      then show ?thesis
      proof(cases "(y1, z1) ∈ lex_L2_word A")
        case True
        then have "z1 ∉ X (SG (F⇘A⇙) H) A" using lex_cont2[of "y1" "z1" "A" "H"] H assms(1) cont by (metis mult_SG)
        moreover have "inv⇘SG F⇘A⇙ H⇙ z1 ∉ X (SG (F⇘A⇙) H) A"  using lex_cont2_inv[of "y1" "z1" "A" "H"] H assms(1) cont True by (metis mult_SG)
        moreover have "X (SG (F⇘A⇙) H) A ⊆ H"  unfolding X_def SG_def by simp
        ultimately have "z1 ∉ (union_inv (X (SG (freegroup A) H) A) A)" using notin_union_inv[of "H" "A" "z1" "X (SG F⇘A⇙ H) A"] assms(1) by simp
