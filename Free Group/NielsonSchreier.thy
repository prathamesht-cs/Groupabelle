...
lemma nat_least_exists:
  ...
  and "0 \<notin> S"
  shows "\<exists>x\<in>(S:: nat set).\<forall>y\<in>S. (x, y) \<in> nat_leq_less "
proof-
  obtain x where "x \<in> S" using assms by auto
  then obtain k where k:"(belongs S k) \<and> (\<forall>i<k. (\<not> (belongs S i))) \<and> k \<le> x" using assms ex_least_nat_le[of "belongs S" x] by (meson belongs_def)
  ...
  proof(rule ballI)
    ...
    qed
  qed
  then show ?thesis using k belongs_def by metis
qed
