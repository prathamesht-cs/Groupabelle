...
lemma "min lex_word (L2 x) = min lex_word (L2 y) \<and> max lex_word (L2 x) = max lex_word (L2 y) \<Longrightarrow> (L2 x = L2 y) \<or> (\<down> (L2 x) = L2 y)" 
...
proof-
  ...
  then show "take ((length xs + 1) div 2 - 1)
     (take ((length (wordinverse xs) + 1) div 2) (wordinverse xs)) =
    wordinverse (drop ((length xs + 1) div 2) xs)" sorry
qed
