...
definition (in group) equiv_inv :: "('a, 'b) monoidgentype set \<Rightarrow> ('a, 'b) word set \<Rightarrow> ('a, 'b) word set"
  where "equiv_inv S w = (reln_set \<langle>S\<rangle> `` {wordinverse (equiv_red S w)})"
