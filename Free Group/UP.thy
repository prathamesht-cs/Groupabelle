...
next
  assume case_asm:"\<forall>w\<in>\<langle>X\<rangle>\<^bsub>G\<^esub>. non_reducible G X w \<longrightarrow> w \<noteq> \<one>\<^bsub>G\<^esub>"
  ...
  proof-
    ...
    have "h w \<noteq> \<one>\<^bsub>G_X\<^esub>"
 ...
 in_set_conv_nth less_imp_le_nat nat_le_linear zero_less_diff) 
    ...
    hence "h w = monoid.m_concat G (map h ls0)" 
      using h(1) hom_preserves_m_concat[OF freegroup_is_group[of S] assms(3) h(1) w_is] 
       in_set_ls0 
      by (meson Un_iff liftgen_inv_in_carrier liftgen_subset subset_iff)
    moreover have "\<forall>x \<in> set (map h ls0). x \<in> X \<union> m_inv G ` X"
    ...
    ultimately have "non_reducible G X (h w)" unfolding non_reducible_def using False len_ls0 
    ...
    then show ?thesis unfolding G_X_def by simp
  qed}
  thus ?thesis by auto
  qed 
  hence h_iso:"h \<in> iso (freegroup S) G_X" using group_hom.group_hom_isoI[OF group_hom_G_X] surj 
    by blast
  then obtain g where g:"group_isomorphisms G_X (freegroup S) g h" 
    using group.iso_iff_group_isomorphisms[OF freegroup_is_group[of S]] h_iso 
    using group_isomorphisms_sym by blast
  hence g_iso:"g \<in> iso G_X (freegroup S)" 
...
(*obtain inverse and use the bijection results to get the final result*)
(*this needs another small theorem*)
  
  have g_map:"g  ` X = (liftgen S)" (*using  bij_betw_inv_into bij_liftgen_X \<phi>_def liftgen_subset[of S]*)
   proof
    {fix y
      assume "y \<in> g ` X"
      hence "y \<in> g ` (h ` (liftgen S))" using bij_liftgen_X unfolding bij_betw_def by meson
    ...
    thus "g ` X \<subseteq> liftgen S" by blast
  next
    {fix y
      assume y:"y \<in> liftgen S"
      hence "h y \<in> X" using bij_liftgen_X unfolding bij_betw_def by blast
      moreover then have "g (h y) = y" using g assms(2) unfolding group_isomorphisms_def G_X_def 
        by (meson liftgen_subset subsetD y)
      ultimately have "y \<in> g ` X" using y by blast}
    thus "liftgen S \<subseteq> g ` X" by auto
  qed

(* iso_set_sym *)
(*defn to keep in mind*)
(*"fg_with_basis (G::('a,'b) monoid_scheme) X 
    \<equiv> (\<exists>(S::(unit \<times> 'a) set) \<phi>. (\<phi> \<in> iso G (freegroup S)) 
              \<and> (\<langle>X\<rangle>\<^bsub>G\<^esub> = carrier G) \<and>(\<phi> ` X = (liftgen S)))"*)
  have "\<langle>X\<rangle>\<^bsub>G\<lparr>carrier := \<langle>X\<rangle>\<rparr>\<^esub> = carrier (G\<lparr>carrier := \<langle>X\<rangle>\<rparr>)" 
  proof-
    have "\<langle>X\<rangle>\<^bsub>G\<lparr>carrier := \<langle>X\<rangle>\<rparr>\<^esub> = \<langle>X\<rangle>" 
    proof
      show  "\<langle>X\<rangle>\<^bsub>G\<lparr>carrier := \<langle>X\<rangle>\<rparr>\<^esub> \<subseteq> \<langle>X\<rangle>" 
        using G_X_def X_subset carrier_eq group.gen_span_closed grp by blast
    next
      show  "\<langle>X\<rangle> \<subseteq> \<langle>X\<rangle>\<^bsub>G\<lparr>carrier := \<langle>X\<rangle>\<rparr>\<^esub>" 
      proof
        fix x
        assume "x \<in> \<langle>X\<rangle>"
        thus "x \<in> \<langle>X\<rangle>\<^bsub>G\<lparr>carrier := \<langle>X\<rangle>\<rparr>\<^esub>"
        proof(induction x rule:gen_span.induct)
          case gen_one
          then show ?case using assms(2,3) 
            by (metis G_X_def carrier_eq gen_span.gen_one group.gen_subgroup_is_subgroup group.l_cancel_one' grp subgroup.mem_carrier subgroup_mult_equality) 
        next
          case (gen_gens x)
          then show ?case 
            by (simp add: gen_span.gen_gens)  
      ...
      qed
    qed
    thus ?thesis using carrier_eq unfolding G_X_def by argo

  qed
  thus "fg_with_basis (G\<lparr>carrier := \<langle>X\<rangle>\<^bsub>G\<^esub>\<rparr>) X"  unfolding fg_with_basis_def
    using g_iso carrier_eq g_map unfolding G_X_def by blast
qed
