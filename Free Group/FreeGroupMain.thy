...
proof(induction xs)
  case Nil
  then show ?case using assms group_spanset.cases group_spanset.empty group_spanset.gen
...
next
  case (Cons a xs)
  then show ?case  using group_spanset.cases  group_spanset.gen  by blast
qed
...
lemma span_wordinverse: assumes "xs \<in> \<langle>S\<rangle>" shows "wordinverse xs \<in> \<langle>S\<rangle>"
  using assms unfolding spanset_def
proof(induction xs)
  case empty
  then show ?case by (simp add: group_spanset.empty)
next
  ...
  then have "[inverse x] \<in> \<llangle>invgen S\<rrangle>" using group_spanset.empty group_spanset.gen by blast
  then have "wordinverse xs @ [inverse x] \<in> \<llangle>invgen S\<rrangle>" using gen  span_append by auto
...
proof-
  ...
  proof- 
    ...
    then show "(y, x) \<in> reln_set \<langle>S\<rangle>" 
      using 1 by (simp add: reln_set_def)
  qed
