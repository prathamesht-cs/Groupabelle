...
next
  ...
  then have " genmap (\<iota> ` gens) f [a] \<in> carrier G" using genmap_closed Cons.prems(1) by auto
...
next
  ...
  then have 1: "genmapext (\<iota> ` gens) f (a # x) \<otimes> genmapext (\<iota> ` gens) f y 
= genmap (\<iota> ` gens) f [a] \<otimes> genmapext (\<iota> ` gens) f (x @ y)" 
    using Cons.IH Cons.prems(1) Cons.prems(3)  a genmap_closed genmapext_closed m_assoc x 
    by metis 
...
lemma cancels_to_1_preserves:
  assumes "cancels_to_1 x y"
      and "x \<in> freewords_on gens"
    shows "y \<in> freewords_on gens"
proof-
  obtain xs1 x1 x2 xs2
  where x:"x = xs1 @ x1 # x2 # xs2"
    and y:"y = xs1 @ xs2"
    using assms cancels_to_1_unfold by metis
  have "xs1 \<in> freewords_on gens" using x leftappend_span freewords_on_def assms(2) by blast
  moreover have "xs2 \<in> freewords_on gens" using x rightappend_span freewords_on_def assms(2) span_cons by blast
...
proof-
  ...
  have x1:"x1 \<in> gens \<times> {True, False}" using  invgen_def gen_spanset freewords_on_def assms(2)  by (metis list.distinct(1) list.sel(1))
  ...
  have x2:"x2 \<in> gens \<times> {True, False}" using  invgen_def gen_spanset freewords_on_def assms(3)  by (metis list.distinct(1) list.sel(1))
...
obtain c1 x1 x2 c2
  ...
  have bin: "b \<in> freewords_on gens" using cancels_to_preserves  cancels_to_def rtrancl_into_rtrancl.prems(1) rtrancl_into_rtrancl.hyps(1)  by metis
  have c1:"c1 \<in> freewords_on gens" using b bin  freewords_on_def leftappend_span by blast
  moreover have fx1:"([x1] @ [x2] @ c2)\<in> freewords_on gens" using b bin  freewords_on_def rightappend_span by fastforce
  moreover then have x1:"[x1] \<in> freewords_on gens" using fx1  freewords_on_def leftappend_span by blast
  moreover  have fx2: "([x2] @ c2) \<in> freewords_on gens" using fx1  freewords_on_def rightappend_span by fastforce
  moreover then have x2:"[x2] \<in> freewords_on gens" using  freewords_on_def leftappend_span by blast
  moreover  have c2: "c2 \<in> freewords_on gens" using fx2  freewords_on_def rightappend_span by fastforce
  ultimately have 2: "genmapext (\<iota> ` gens) f (c1 @ [x1] @ [x2] @ c2) 
              = genmapext (\<iota> ` gens) f c1  \<otimes> (genmapext (\<iota> ` gens) f [x1] \<otimes> genmapext (\<iota> ` gens) f [x2]) \<otimes> genmapext (\<iota> ` gens) f c2" using genmapext_append rtrancl_refl.prems(3)
    by (smt (z3) genmapext_closed m_assoc m_closed)
  then have "genmapext (\<iota> ` gens) f (c1 @ [x1] @ [x2] @ c2) = genmapext (\<iota> ` gens) f c1  \<otimes> \<one> \<otimes> genmapext (\<iota> ` gens) f c2" 
    using inverse_ext i x1 x2 assms(4) sorry
...
lemma (in group) genmapext_reln_tuple:
  ...
  shows "genmapext (\<iota> ` gens) f x = genmapext (\<iota> ` gens) f y"
proof-
  let ?rx = "iter (length x) reduce x"
  let ?ry = "iter (length y) reduce y"
  have "cancels_to x ?rx" by (simp add: iter_cancels_to)
  moreover then have "?rx  \<in> freewords_on gens" using assms cancels_to_preserves by blast
  ...
  moreover then have ryg:"?ry  \<in> freewords_on gens" using assms cancels_to_preserves by blast
...
next
  ...
  then show ?case
    proof(cases "cancels_to b c")
      case True
      then have b:"b \<in> freewords_on gens" using rtrancl_into_rtrancl.prems(1)  reln_tuple_def  sledgehammer
...
proof-
  ...
  proof-
    ...
    ultimately show "(genmapext (\<iota> ` S) f x) = (genmapext (\<iota> ` S) f y)" 
      using genmapext_reln_tuple assms by blast
  qed
...
proof-
  ...
  then have "genmapext_lift (\<iota> ` S) f (x \<otimes>\<^bsub>(freegroup S)\<^esub> y) 
= (genmapext_lift (\<iota> ` S) f x) \<otimes> (genmapext_lift (\<iota> ` S) f y)"
    using genmapext_lift_wd x2 x1 y2 y1  assms(1) by metis
}
...
proof-
  ...
  then have "x \<otimes>\<^bsub>(freegroup S)\<^esub> (reln_tuple \<langle>S\<rangle>) `` {wordinverse x1} = ((reln_tuple \<langle>S\<rangle>) `` {x1@wordinverse x1})" using proj_append_wd by (simp add: proj_append_wd 1 2)
  ...
  then moreover have "proj_append \<langle>S\<rangle>  ((reln_tuple \<langle>S\<rangle>)``{wordinverse x1}) ((reln_tuple \<langle>S\<rangle>)``{x1}) = reln_tuple \<langle>S\<rangle> `` {(wordinverse x1)@x1}"  by (simp add: 1 2 proj_append_wd)
...
next
  ...
  proof(cases "[a]  \<in> (\<iota> ` S)")
    ...
    then have "(reln_tuple \<langle>S\<rangle> `` {[a]}) \<in> \<langle>liftgen S\<rangle>\<^bsub>(freegroup S)\<^esub>"  by (simp add: gen_span.gen_gens)
...
next
  ...
  then have 1: "(reln_tuple \<langle>S\<rangle> `` {wordinverse [a]}) \<in> \<langle>liftgen S\<rangle>\<^bsub>(freegroup S)\<^esub>"  by (simp add: gen_span.gen_gens)
